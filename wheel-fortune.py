...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        ...
        for ltr in LETTERS:
            if ltr not in guessed and ltr not in VOWELS:
                possibleLetters.append(ltr)
            elif ltr not in guessed and ltr in VOWELS:
    ...
    def getMove(self, category,obscuredPhrase, guessed):
        getMoveList = self.getPossibleLetters(guessed)
        
        if getMoveList == []:
            return 'pass'
        else:
            if self.smartCoinFlip == True:
                for ltr in self.SORTED_FREQUENCIES:
                    for ltr in getMoveList:
                        return ltr
            else:
                return random.choice(getMoveList)
...
def requestPlayerMove(player, category, guessed):
    while True: # we're going to keep asking the player for a move until they give a valid one
        ...
        move = player.getMove(category, obscurePhrase, guessed)
